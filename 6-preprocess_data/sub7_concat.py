...
# ----- 경로 설정 -----
BASE = "/nas/research/03-Neural_decoding/3-bids/2-derivatives/1-beta/beta_nsd/sub-07"
nii_dir = BASE
responses_tsv = os.path.join(BASE, "responses.tsv")
stim_csv = os.path.join(BASE, "subject7_stim_info.csv")

out_train = os.path.join(BASE, "sub-07_beta_train.nii.gz")
out_test  = os.path.join(BASE, "sub-07_beta_test.nii.gz")
...
assert "num" in stim.columns and "shared1000" in stim.columns, "subject7_stim_info.csv에 num/shared1000 컬럼이 없습니다."
...
for i, k in enumerate(kid_seq):
    ...
    if v is None:
        raise ValueError(f"subject7_stim_info.csv에 num={k}가 없습니다.")
    cat[i] = 1 if v else 0
